...
module%test Effect = struct
  ...
  let%expect_test "Confirm waits for dismissal to resolve false" =
    let%with handle =
      Handle.with_async ~get_vdom:Fn.id ~filter_printed_attributes (fun graph ->
        let open Bonsai.Let_syntax in
        let confirm = Skyline_confirm_dialog_v2.effect graph in
        let effect_chain =
          let%arr confirm in
          let%bind.Effect () = Effect.print_s [%message "start chain"] in
          let%bind.Effect result = confirm ~title:"Confirm" {%html|This is a confirm|} in
          Effect.print_s [%message "end chain" (result : bool)]
        in
    ...
    [%expect {| ("end chain" (result false)) |}];
    Deferred.return ()
  ;;

  let%expect_test "Confirm waits for resolution to resolve" =
    let confirm_selector = Test_selector.make () in
    let%with handle =
      Handle.with_async ~get_vdom:Fn.id ~filter_printed_attributes (fun graph ->
        let open Bonsai.Let_syntax in
        let confirm = Skyline_confirm_dialog_v2.effect graph in
        let effect_chain =
          ...
          let%bind.Effect result =
            confirm
              ~confirm_test_selector:confirm_selector
              ~title:"Confirm"
              {%html|This is a confirm|}
          in
          Effect.print_s [%message "end chain" (result : bool)]
        in
        Bonsai.Edge.lifecycle ~on_activate:effect_chain graph;
    ...
    Handle.click_on handle ~selector:(test_selector confirm_selector);
    Handle.one_frame handle;
    Handle.print_active_element handle;
    [%expect {| <body> ... </body> |}];
    (* We need to unroll the async queue to be able to continue the effect chain to its
  ...
  let%expect_test "The confirm button gets focused at modal opening" =
    ...
    let%with handle =
      Handle.with_ ~get_vdom:Fn.id ~filter_printed_attributes (fun graph ->
        ...
        let on_click _ =
          confirm
            ~confirm_label:"Confirm button within the dialog"
            ~title:"Confirm"
            {%html|Example confirm!|}
          |> Effect.ignore_m
        in
        [%html
          {|
